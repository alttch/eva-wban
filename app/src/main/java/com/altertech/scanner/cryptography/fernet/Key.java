...
import static com.altertech.scanner.cryptography.fernet.Constants.signingAlgorithm;
import static com.altertech.scanner.cryptography.fernet.Constants.signingKeyBytes;
import static com.altertech.scanner.cryptography.fernet.Constants.tokenPrefixBytes;
...
public class Key {
     ...
     *
     * @param signingKey    a 128-bit (16 byte) key for signing tokens.
     * @param encryptionKey a 128-bit (16 byte) key for encrypting and decrypting token contents.
     */
    ...
    /**
     * @param concatenatedKeys an array of 32 bytes of which the first 16 is the signing key and the last 16 is the
    ...
    public static Key generateKey(final Random random, byte[] encryptionKey) {
        final byte[] signingKey = new byte[signingKeyBytes];
        random.nextBytes(signingKey);
        return new Key(signingKey, encryptionKey);
    }

    public static Key generateKey(byte[] signingKeyBytes, byte[] encryptionKey) {
        return new Key(signingKeyBytes, encryptionKey);
    }
